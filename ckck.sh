...
Get_Full_Name(){
    ...
    tmp_NickName_2="$(def_json_value "$dir_scripts/CK_WxPusherUid.json" NickName "pin=$j;")"
...
verify_ck(){
    ...
    check_validity(){
        ...
        if [[ ${ck_status[$j]} = 0 ]]; then
            ...
            last_validity_day[$j]="$(def_json_value "$dir_scripts/CK_WxPusherUid.json" validity_day "pin=$j;" | head -1)"
